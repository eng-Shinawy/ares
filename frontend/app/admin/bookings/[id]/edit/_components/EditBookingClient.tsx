...
interface BookingResponse {
  _id: string;
  supplier?: { _id: string };
  pickupLocation?: { _id: string };
  dropOffLocation?: { _id: string };
  from: string;
  to: string;
...
export default function EditBookingClient({ bookingId }: Readonly<{ bookingId: string }>) {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const booking = await apiFetchJson<BookingResponse>(`api/booking/${bookingId}/en`, {
          accessToken: session.accessToken,
        });

        // ج. تعبئة الخيارات الحالية عشان الـ Selects ما تضربش أخطاء
        if (booking.driver) {
          setUsers([booking.driver]);
        }
        if (booking.car) {
          setCars([booking.car]);
        }
    ...
    if (!isLoadingInitial && supplier && pickupLocation && from && to && session?.accessToken) {
      ...
      try {
        ...
        if (currentCarId && !availableCars.find((c: Car) => c._id === currentCarId)) {
          ...
          if (currentCarObj) {
            availableCars = [currentCarObj, ...availableCars];
          }
        }
  ...
  }, [formData, isLoadingInitial, session?.accessToken, cars]);
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Cannot cancel this booking.";
      alert(message);
    } finally {
