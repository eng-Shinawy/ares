...
export function useLocationFormApi({
	...
	const attachImage = useCallback(
		async (token: string, locId: string, file: File): Promise<boolean> => {
			try {
	...
	const submitForm = useCallback(
		async (
			...
			try {
				const resultLocation = await saveLocation(
					formData,
					userId,
					accessToken,
				);
				...
				if (imageFile) {
					const uploaded = await attachImage(
						accessToken,
						resultLocation.id,
						imageFile,
					);
					if (!uploaded && mode === "create") {
						callbacksRef.current.onError(
							messagesRef.current.imageUploadFailedMessage,
						);
						return false;
					}
				}
				...
				callbacksRef.current.onError(
					parseApiError(err, messagesRef.current.errorMessage),
				);
				return false;
			} finally {
				setLoading(false);
