...
interface VehicleInfoProps {
  ...
  readonly vehicleId: string; // محتاجينه عشان الـ API بتاع المفضلات
}
...
export default function VehicleInfo({ vehicle, vehicleId }: VehicleInfoProps) {
  ...
  const handleFavorite = async () => {
    ...
    try {
      const response = await fetch(`/api/vehicles/${vehicleId}/favorites`, {
        method: "POST",
      });
      if (response.ok) {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="space-y-3">
        <h3 className="text-lg font-bold text-slate-900 transition-colors duration-300 dark:text-white">About this vehicle</h3>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-slate-900 transition-colors duration-300 dark:text-white">Key Specifications</h3>
