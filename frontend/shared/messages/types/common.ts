export type CommonLabels = {
  readonly title: string;
  readonly description: string;
  readonly welcome: string;
  readonly toggleTheme: string;
  readonly languageSwitcher: string;
  readonly langAr: string;
  readonly langEn: string;
  readonly langArShort: string;
  readonly langEnShort: string;
  readonly themeDark: string;
  readonly themeLight: string;
  readonly notifications: string;
  readonly retry: string;
  readonly loading: string;
  readonly save: string;
  readonly cancel: string;
  readonly delete: string;
  readonly edit: string;
  readonly search: string;
  readonly filter: string;
  readonly export: string;
  readonly import: string;
  readonly submit: string;
  readonly confirm: string;
  readonly back: string;
  readonly next: string;
  readonly previous: string;
  readonly close: string;
  readonly yes: string;
  readonly no: string;
  readonly actions: string;
  readonly status: string;
  readonly active: string;
  readonly inactive: string;
  readonly all: string;
  readonly none: string;
  readonly selectAll: string;
  readonly deselectAll: string;
  readonly profile: string;
  readonly logout: string;
  readonly signOut: string;
  readonly signIn: string;
  readonly register: string;
  readonly dashboard: string;
  readonly theme: string;
  readonly language: string;
  readonly currency: string;
  readonly searchPlaceholder: string;
  readonly switchToDarkMode: string;
  readonly switchToLightMode: string;
  readonly markAllAsRead: string;
  readonly viewAllNotifications: string;
  readonly noNotificationsYet: string;
  readonly failedToLoadNotifications: string;
  readonly tryAgain: string;
  readonly notificationDeletedSuccessfully: string;
  readonly failedToDeleteNotification: string;
  readonly allCaughtUp: string;
  readonly unreadMessages: string;
  readonly justNow: string;
  readonly minutesAgo: string;
  readonly hoursAgo: string;
  readonly daysAgo: string;
};
