...
flowchart TD
    ...
    Client([Client\nAuthenticated user\nRoles: Customer, Supplier, Driver, Inspector])
    Admin([Admin\nPrivileged user])
    ...
    %% System
    Ares_Car_Rental[Ares Car Rental System\nCore Application]

    %% Interactions
    Guest -->|Browse vehicles\nView public info| Ares_Car_Rental
    Guest -->|Register\nLogin| Ares_Car_Rental
    Client -->|Book vehicles\nManage profile\nUpload documents| Ares_Car_Rental
    Admin -->|Verify users\nApprove vehicles\nManage bookings| Ares_Car_Rental
    Ares_Car_Rental -->|Send emails| AWS_SES
    Ares_Car_Rental -->|Store files| AWS_S3
    Ares_Car_Rental -->|Process payments\nHandle refunds| Payment_Gateway
    Payment_Gateway -->|Payment confirmation| Ares_Car_Rental
```
...
flowchart TD
    ...
    Draft -->|Driver Required| DriverSelection{Driver\nSelection}
    Draft -->|No Driver| BeginPayment[Begin Payment\nState = PaymentPending]
    DriverSelection -->|Select Driver| DriverSelected[Driver Selected\nState = DriverSelected]
    DriverSelected --> BeginPayment
    BeginPayment --> ConfirmPayment{Confirm\nPayment}
    ConfirmPayment -->|Success| Confirmed[Confirmed\nUpdate Booking\nState = Confirmed\nCreate Payment]
    ConfirmPayment -->|Failure| PaymentFailed[Payment Failed\nState = PaymentPending]
    PaymentFailed --> BeginPayment
    Confirmed --> End([End])
    Draft -->|Cancel| Cancelled[Cancelled\nState = Cancelled\nDelete Booking]
    DriverSelected -->|Cancel| Cancelled
    BeginPayment -->|Cancel| Cancelled
    Cancelled --> End
```
...
flowchart TD
    ...
    ReceiveImage --> ValidateImage{Validate\nSize & Format}
    ValidateImage -->|Valid| GenerateFilename[Generate Unique\nFilename]
    ValidateImage -->|Invalid| Error[Return 400\nBad Request]
    GenerateFilename --> ResizeImage[Resize Image\nThumbnail, Medium, Large]
    ResizeImage --> CompressImage[Compress Image]
    CompressImage --> UploadToS3[Upload to AWS S3]
    UploadToS3 --> PersistURL[Persist URL\nUpdate Database]
    PersistURL --> ReturnURL[Return Image URLs\n200 OK]
    ReturnURL --> End([End])
    Error --> End
```
...
```mermaid
flowchart TD
    Start([Event Occurs]) --> PublishEvent[Publish Event\nBookingConfirmedEvent]
    PublishEvent --> GenerateNotification[Generate Notification\nCreate Notification Record]
    GenerateNotification --> SendEmail{Email\nRequired?}
    SendEmail -->|Yes| PrepareEmail[Prepare Email\nAWS SES Template]
    PrepareEmail --> SendViaSES[Send Email\nAWS SES]
    SendEmail -->|No| DeliverInApp[Deliver In-App\nNotification]
    SendViaSES --> DeliverInApp
    DeliverInApp --> End([End])
```
...
```mermaid
flowchart TD
    Start([Booking Created]) --> FilterDrivers[Filter Available Drivers\nIsAvailable = true\nStatus = Approved]
    FilterDrivers --> CheckAvailability[Check Availability Window\nNo Overlapping Bookings]
    CheckAvailability --> CalculateProximity[Calculate Proximity\nHaversine Formula]
    CalculateProximity --> FilterByDistance[Filter by Distance\n<= 20 km]
    FilterByDistance --> SortDrivers[Sort by Proximity\nand Rating]
    SortDrivers --> SelectDriver[Select Top Driver\nAssign to Booking]
    SelectDriver --> NotifyDriver[Notify Driver\nIn-App Notification]
    NotifyDriver --> End([End])
```
...
```mermaid
flowchart TD
    Start([Cancel Booking]) --> CheckCancellationWindow[Check Cancellation Window\nTime Until Pickup]
    CheckCancellationWindow -->|> 24h| CalculateFullRefund[Calculate Full Refund\nTotal Price - Driver Fee]
    CheckCancellationWindow -->|<= 24h| CalculatePartialRefund[Calculate Partial Refund\nTotal Price - Cancellation Fee - Driver Fee]
    CalculateFullRefund --> ProcessRefund[Process Refund\nPayment Gateway]
    CalculatePartialRefund --> ProcessRefund
    ProcessRefund --> UpdateRecords[Update Payment & Booking\nStatus = Refunded/Cancelled]
    UpdateRecords --> SendConfirmation[Send Confirmation Email\nAWS SES]
...
flowchart TD
    %% Steps
    VehicleStep[Vehicle Step\nDisplay Vehicle Summary]
    DriverStep[Driver Step\nSelect Driver or Self-Drive]
    PaymentStep[Payment Step\nEnter Payment Details]
    ConfirmationStep[Confirmation Step\nReview Booking]

    %% Navigation
    VehicleStep -->|Next| DriverStep
    DriverStep -->|Back| VehicleStep
    DriverStep -->|Next| PaymentStep
    PaymentStep -->|Back| DriverStep
    PaymentStep -->|Next| ConfirmationStep
    ConfirmationStep -->|Back| PaymentStep
    ConfirmationStep -->|Confirm| SuccessPage[Success Page\nBooking Confirmed]
```
